# ppo_fly_rec.py -- FLY (predict + send UART action) AND RECORD at the same time, for PPO h64.
# Same deploy config as ppo_main.py (feedforward, 64x64x3 stack, persistent buffer, no filtering).
# Records a fixed MISSION_S-second window that STARTS on the first UART goal; logs the per-frame
# single 64x64 obs (rebuild 3-stacks offline), goal, and action -- so the log is BOTH a flight
# trace AND a task-real calibration source (feed to recalibrate_int8.py after rebuilding stacks).
#
...
KMAX = 4000                               # safety cap if UART never arrives

model = ml.Model(MODEL); print('ppo_fly_rec: model in', model.input_shape)
...
# persistent stack buffer (no per-frame 49KB alloc), newest -> last channel (matches training)
_img = np.zeros((1, G, G, KSTACK)); _first = True
def build_img():
    global _first
    d = _c.snapshot()
    _g.draw_image(d, 0, 0, x_scale=SC, y_scale=SC, hint=image.AREA)
    ob = _g.bytearray()                                 # uint8 newest frame (for logging)
...
goal_vec = [1.0, 0.0, 0.0, 0.5]           # CF: [cos(brg), sin(brg), yaw_rate/3.5, |v_EKF|/vmax(=3.0)]
...
print('ppo_fly_rec armed -> waiting for first UART, then flying + logging %ds' % MISSION_S)
gc.collect()
